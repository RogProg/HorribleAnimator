...
static AVFrame* get_audio_frame(OutputStream* ost)
{
    ...
    for (j = 0; j < frame->nb_samples; j++) {
        ...
        if (soundpos < wavlength) {

            q[j * 2] = audio[soundpos++];
            q[j * 2 + 1] = audio[1 + soundpos++];
...
static AVFrame* get_video_frame(OutputStream* ost)
{
    ...
    */
    ...
    sws_scale(ctx, &video, inLinesize, 0, height, ost->frame->data, ost->frame->linesize);
